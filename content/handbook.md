+++
title = "Handbook"
type = "docs"
weight = 10
eyebrow = "Handbook"
lead = "An open guide to how our community works: whether you'd like to speak at a meetup, host a venue, donate, or help organize, you'll find what you need here. For the step-by-step organizing guides, see the Meetup Runbooks."
+++

## Welcome

Welcome to the PyLadies Vancouver Handbook! We're publishing this in the open for two
reasons: so anyone helping out has a clear, repeatable guide to follow, and so our
community can see how much goes into each event.

{{% callout tone="info" title="Anyone can help" %}}
You don't need to be a designated "organizer" to pitch in. We coordinate in the
**#city-vancouver** channel in the [PyLadies Slack](https://slackin.pyladies.com): come
say hi, tell us what you'd like to help with, and we'll get you started. You can also
reach us at **vancouver at pyladies dot com**.
{{% /callout %}}

## Ways to participate

Each guide is its own page:

- **[Speak at our meetup](/speak/)**: who should speak, talk formats, how to submit, and
  how to prepare
- **[Host a venue](/host/)**: what we need from a space, what's optional, and how to
  offer one
- **[Donate and sponsor](/donate/)**: supporting PyLadies Vancouver directly or
  sponsoring an event
- **[Help organize](/organize/)**: ways to pitch in, no special access required

## For organizers and contributors

- **[Meetup Runbooks](/runbooks/)**: the behind-the-scenes steps we follow to run a
  meetup, from finding a date to publishing the recap
- **[Contributing to the Website](/contributing/)**: how to add blog posts, events, and
  pages to this site

## Stay up to date

Keep up with what we're doing:

- **Subscribe to our [Luma calendar](https://api.luma.com/ics/get?entity=calendar&id=cal-EefnU0O5BifyHgg)**
  to get upcoming events in your own calendar app
- **Subscribe to our <a href="/index.xml">RSS feed</a>** for new blog posts
- **Follow us on social media**: LinkedIn, Instagram, Mastodon, and Bluesky (linked below)
- **Follow our hashtag**: **#pyladiesvancouver**

## Get in touch

- **Slack**: the **#city-vancouver** channel on the [PyLadies Slack](https://slackin.pyladies.com)
- **Email**: **vancouver at pyladies dot com**

You can also find us on
[LinkedIn](https://linkedin.com/company/pyladies-vancouver),
[Instagram](https://www.instagram.com/pyladiesyvr),
[Mastodon](https://fosstodon.org/@pyladies_vancouver), and
[Bluesky](https://bsky.app/profile/pyladiesvancouver.bsky.social).
